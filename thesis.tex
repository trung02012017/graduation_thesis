...
=======
>>>>>>> dfe4c65f2241a50093b7448d14434f48ab570cbd
\begin{document}
...
enhanced over the course of iteration by the candidate solutions.  Several other SI algorithms have been regularly proposed, some of them are listed in Table ***. In general, this group of methods started to become more attractive since PSO is proven to be very competitive with evolution-based and physics-based algorithms. In fact, swarm-based methods have some advantages over the others. For example, swarm-based algorithms find new better by preserving the information from previous iterations, while evolution-based methods such as GA discard any information immediately when a new generation is formed. Also, they are usually formed of less updating operators compared to the others (crossover, mutation, selection $etc.$) and therefore it is easier to implement.
	...
	Although a wide range of evolutionary and swarm-based optimization algorithms are developed and applied to optimizing ANNs, the problem of local minima still remains. According to the no-free-luch theorem (NFL), there is no superior method that can solving all optimization issues. Motivated by this reason, in this work, beside enhacing SLnO to ISLO algorithm, a model named ISLO-CFNN, which is a combination of ISLO optimizer and Cascade Feedforward Neural Network (CFNN) \cite{schetinin2003learning} architecture, is proposed for solving time-series prediction problem. The issue ISLO-CFNN tries to tackle stems from the auto-scaling demand in cloud environments, which requires cloud servers unable to be adaptive and scalable, automatically recovering and allocating resources effectively. Our proposed ISLO-CFNN model is tested by the experiments on 4 datasets. The results provided by the model are compared with well-known deep learning neural networks that are widely used for time-series forecasting systems such as original Recurrent Neural Network (RNN), Long Short Term Memory (LSTM) and Gated Recurrent Unit (GRU) in terms of accuracy and prediction time. Also, the performance of ISLO in optimizing capability on CFNN architecture is tested in this experiment by comparing with 2 swarm-based algorithms: SLnO and PSO, and the fundamental GD algorithm, working on time-series data. The figures obtained show that ISLO-CFNN is very competitive with the others, providing potential results when solving the time-series prediction problem.
	
	This thesis outline is organized as follows: the fundamental knowledge about swarm-based algorithms, ANNs and auto-scaling issues in cloud computing is presented in Chapter \ref{ch:background}. Chapter \ref{ch:improvements} describes our two main improvements on original SLnO algorithm, and the proposed model for tackling the above mentioned problem. Our experiments for testing the performance of ISLO algorithm and ISLO-CFNN model are discussed in Chapter \ref{ch:experiments}. Finally, the contribution and future work of this research are concluded in Chapter \ref{ch:conclusion}. 
